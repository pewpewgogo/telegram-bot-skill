...
name: agntdev-deploy
description: >
  Use when making an AGNTDEV-PLATFORM bot repo deployable, debugging why a deployed
  bot won't start on the agnt-gm pipeline, or deciding what build/deploy files to
  commit. Platform-specific tool tier — for generic webhook/polling/hosting deploy
  see telegram-bot-deploy instead.
  Triggers: agntdev deploy, agnt-gm deploy, platform deploy, dist/index.js,
  dist/main.js, BOT_TOKEN_FILE, REDIS_URL, bot starter template, container_state,
  fly.io, docker compose, VPS, crash loop, GitHub Packages, NODE_AUTH_TOKEN, .npmrc,
  @agntdev/bot-toolkit deploy.
compatibility: agnt-gm platform only. Node 20+, npm. No deploy credentials needed — the platform builds and deploys for you.
license: MIT
---

# agntdev-deploy Skill

> **Platform tool tier.** This skill is the deploy contract for the **agnt-gm /
> agntdev** pipeline specifically. If you're not on that platform, you want the
> generic [telegram-bot-deploy](../telegram-bot-deploy/SKILL.md) skill (webhook vs
> polling, hosting, serverless) instead.
